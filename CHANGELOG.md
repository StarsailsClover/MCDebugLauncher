...
Fourth official release. Collapses the v26.3 mainline (Alpha 1 through
Alpha 10). Theme: hardening & agent surface completion, driven by the
v26.2 robustness assessment (ROBUSTNESS_V262.md) and a workspace-wide
blocker scan of sibling Minecraft projects.

Highlights across the line:
- **Alpha 1 - Input/config hardening**: instance-name validation
  (reserved Windows device stems, separators); BOM-tolerant JSON readers
  with file-path errors; attach triage (non-JVM vs missing module).
- **Alpha 1 - Agent API phase 2**: `instance/:i/metrics` + `/disk`
  endpoints; execute `metrics`/`disk`/`inject-agent`/`server-cmd`.
- **Alpha 2 - OOM second confirmation**: candidates listed with PID /
  window title / memory before termination; `--oom-confirm
  auto|always|never`; `--oom-list-only` dry run; platform matrix doc.
- **Alpha 3 - Diagnostics correlation**: diagnose embeds idle-watchdog
  events + last-launch metrics; evidence-backed correlation notes.
- **Alpha 4 - Server deepening**: comment/order-preserving
  server.properties editor (`mdl server props`); allowlist/op/ban
  wrappers with stopped-server file fallbacks.
- **Alpha 5 - Security + OOM false-kill fix**: account-token files ACL-
  hardened (current-user only); `mdl server rotate-rcon`; strong launch
  markers end compile-daemon/IDE-build false kills in `…Minecraft…`
  workspaces.
- **Alpha 6 - Performance baseline**: `mdl bench` CLI cold-latency
  benchmark; scripts/perf-bench.ps1 baseline/gate; status-p95 finding
  tracked.
- **Alpha 7 - Docs refresh**: AGENT_API rewritten (12 endpoints / 9
  commands / 8 events / 11 codes); specification Agent section
  de-fictionalized; examples rewritten and syntax-checked.
- **Alpha 8 - Watchdog race fix + adversarial suite**: deferred arming
  under --wait-ready (OpenLumin field blocker); garbage-corpus parser
  tests; C-drive exhaustion incident disclosed.
- **Alpha 9 - Usability**: doctor duplicate-install detection; `--mc`
  alias matching sibling runbooks.
- **Alpha 10 - LTS convergence**: banners/changelog finalized; full
  regression pass.

## [26.2.0] - 2026-08-22

### v26.2 Official Release

Third official release. Collapses the entire v26.2 mainline (Alpha 1 through
Alpha 10) into a stable, non-prerelease build. Theme: automation resilience
and operations for unattended / agent-driven workflows.

Highlights across the line:
- **Alpha 1 - Idle watchdog**: detached games auto-stop after N seconds
  without log output (`--idle-timeout`, default 60s; `game_idle_timeout`
  WebSocket event; `GET /api/v1/game/:instance/idle-status`).
- **Alpha 2 - Diagnostics + performance**: mclog-analyzer parser integrated
  (MC + PCL2 formats, crash types, mod correlation); stack traces extracted
  from crash reports; streaming downloads with disk SHA1 verification cut
  first-launch peak memory from ~1.9GB to <100MB.
- **Alpha 3 - Agent surface convergence**: dead placeholder modules removed;
  capability manifest synced with actual endpoints/events/error codes.
- **Alpha 4 - OOM self-protection**: stale MC/Java process cleanup, working
  set trim, optional standby-list purge before spawn (`--oom-protect`,
  `--oom-aggressive`).
- **Alpha 5 - JavaAgent at launch + mrpack export**: repeatable
  `--javaagent jar[=params]`; instance-level registry (`mdl javaagent`);
  Modrinth `.mrpack` export completing the import roundtrip.
- **Alpha 6 - Hot-attach injection + account refresh**: `mdl game
  inject-agent` loads agents into RUNNING games via jdk.attach (agentmain);
  Microsoft refresh tokens persisted and renewable (`mdl account refresh`).
- **Alpha 7 - Server automation**: RCON client (auto-configured at create),
  graceful stop with kill fallback, `--wait-ready` log polling, console
  commands via `mdl server cmd`; adaptive test-world entry completed.
- **Alpha 8 - Aprism ecosystem**: unified offline status view; native `.aje`
  mods in mod management; zero warnings; debug-build help stack overflow fixed.
- **Alpha 9 - Observability + Refract/Prismate lifecycle**: per-launch
  metrics (`mdl metrics`, local-only), structured JSON logging
  (`--log-format json`), remove commands for Refract/Prismate.
- **Alpha 10 - LTS convergence**: documentation refresh and regression pass.
